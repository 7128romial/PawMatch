...
- SINGLE-DIRECTION QUESTIONS (Critical): Each Active Parameter must be probed with a CLOSED, single-direction question whose short answer maps deterministically to ONE 1-5 value. NEVER ask a disjunctive "X או Y?" / "X or Y?" question for an Active Parameter (e.g. do NOT ask "will someone walk the dog OR will you be home?"), because a bare "כן"/"yes" then maps to opposite values and cannot be resolved. Instead ask for the concrete quantity/fact directly (e.g. "כמה שעות בערך הכלב יישאר לבד בבית ביום ממוצע?"), so that a number, "כן", or "לא" yields a single, unambiguous value.
